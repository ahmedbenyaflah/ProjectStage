...
def process_logs(target_date):
    ...
    print(f"\n🚀 STARTING LOG ENGINE | Date: {target_date}")
            ...
            with open(f_path, 'r', errors='ignore') as f:
                for line in f:
                    ...
                    if qid not in journeys:
                        journeys[qid] = {
                            "qid": qid, "sender": None, "recipients": [],
                            "serverPath": [server_name], "deliveryId": None,
                            "relayIp": None, "status": "Pending", "date": target_date,
    ...
    for qid, j in journeys.items():
        if j["audit"]["fes_lines"]:
            ...
            actions.append({"_index": f"mail-journeys-{target_date}", "_id": qid, "_source": j})
    ...
    if actions:
        ...
        print(f"✅ {target_date} | Success: {stats['Success']} | Failed: {stats['Failed']} | Discarded: {stats['Discarded']} | Time: {elapsed}s")

if __name__ == "__main__":
    # 1. Get all available dates from the FES log folder
    log_path = "../Log-CG/FES01/*.log"
    all_log_files = glob.glob(log_path)
    # Extracts YYYY-MM-DD from the filename (assuming standard naming convention)
    available_dates = sorted(list(set([os.path.basename(f)[:10] for f in all_log_files])))

    # 2. Identify which indices already exist in Elasticsearch
    try:
        # Get all indices matching the pattern
        existing_indices = es.indices.get_alias(index="mail-journeys-*").keys()
    except Exception:
        existing_indices = []

    # 3. Process only the dates that don't have an index yet
    for date_str in available_dates:
        index_name = f"mail-journeys-{date_str}"
